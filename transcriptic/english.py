...
class AutoprotocolParser(object):
    ...
    def fluorescence(self, opts):
        return ("Read fluorescence of %s of plate %s at "
                "excitation wavelength %s and "
                "emission wavelength %s" % (self.well_list(opts['wells']),
                                            opts['object'],
    ...
    def stamp(self, opts):
        ...
        for g in opts['groups']:
            for pip in g:
                if pip == "transfer":
                    stamps.extend(
                        ["Stamp %s from source origin %s "
                         "to destination origin %s %s (%s)" %
                         (self.unit(p['volume']),
                          p['from'], p['to'],
                          ("with the same set of tips as previous"
                              if (len(g[pip]) > 1 and i > 0) else ""),
                          ("%s rows x %s columns" % (g['shape']['rows'],
                           g['shape']['columns']))) for i, p in
                            enumerate(g[pip])])
        return stamps
        ...
        for g in opts['groups']:
            for pip in g:
                if pip == "mix":
                    for m in g[pip]:
                        pipettes.append(
                            "Mix well %s of plate %s %d times with "
                            "a volume of %s" % (
                                self.well(m['well']),
    ...
    def unit(u):
        ...
        return "%s %s" % (value,
                          (unit + "s" if (
                              float(value) > 1 and unit in PLURAL_UNITS)
                              else unit
                           ))

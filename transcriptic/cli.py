...
def upload_release(ctx, archive, package):
    ...
    with click.progressbar(None, 100, "Upload Progress",
        ...
        time.sleep(10)
        status = ctx.obj.get(
            'packages/%s/releases/%s?_=%s' % (package_id, re, int(time.time())))
    ...
    if id:
        ...
        for r in req['runs']:
            runs.append([r['title'] or "(Untitled)",
                         r['id'],
                         r['completed_at'].split(
                             "T")[0] if r['completed_at'] else r['created_at'].split("T")[0],
                         r['status'].replace("_", " ")])
...
def delete_project(ctx, name, force):
    ...
    if id:
        if not force:
            click.confirm(
                "Are you sure you want to permanently delete '%s'?" % get_project_name(
                    id),
                default=False,
                abort=True
            )
...
def price(response):
    ...
    for quote_item in response['quote']['items']:
        click.echo("  %s: %s" % (
            quote_item["title"], locale.currency(float(quote_item["cost"]), grouping=True)))
...
def preview(ctx, protocol_name, view):
    ...
    try:
        ...
        click.echo(
            "Error: Your manifest.json file does not have a \"command_string\" key.")
        return

...
def submit(ctx, file, project, title, test):
    ...
    ----------
    file : file
        File containing raw Autoprotocol-formatted JSON.
    project : str
        Project name or ID to submit JSON to.
    title : str, optional
        Optional title to associate with run (if not specified the title will
        default to the run id generated by the server)
    test : bool
        Specify `--test` to submit the run in test mode.
